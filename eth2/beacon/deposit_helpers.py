...
from eth2.beacon.typing import ValidatorIndex, Gwei


def add_pending_validator(
    state: BeaconState, validator: Validator, amount: Gwei
) -> BeaconState:
    """
...
def validate_deposit_order(state: BeaconState, deposit: Deposit) -> None:
    """
...
def process_deposit(
    ...
    if pubkey not in validator_pubkeys:
        ...
        state = state.update_validator_balance(
            validator_index=index, balance=state.validator_balances[index] + amount
        )
