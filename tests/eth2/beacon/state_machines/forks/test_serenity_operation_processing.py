...
def test_process_max_attestations(
    ...
    attestations = create_mock_signed_attestations_at_slot(
        state=state,
        config=config,
        state_machine=fixture_sm_class(BeaconChainDB(base_db), genesis_block),
...
def test_process_attester_slashings(
    ...
    if success:
        ...
        # Check if slashed
        assert (
            new_state.validator_balances[attester_index]
...
@pytest.mark.parametrize(
    ...
    [(10, 2, 1, 2, 2, True, 0), (10, 2, 1, 2, 2, False, 0), (40, 4, 2, 3, 5, True, 0)],
)
def test_process_attestations(
    ...
    if not success:
        ...
        invalid_attestation_data = attestations[-1].data.copy(slot=state.slot + 10)
