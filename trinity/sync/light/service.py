...
from typing import Any, Callable, cast, Dict, List, FrozenSet, Type
...
from p2p.exceptions import BadLESResponse, NoConnectedPeers, NoEligiblePeers
...
from p2p.service import BaseService, service_timeout
...
class LightPeerChain(PeerSubscriber, BaseService, BaseLightPeerChain):
    ...
    def __init__(
        ...
        self._pending_replies: Dict[
            int, Callable[[protocol._DecodedMsgType], None]
        ] = {}
    ...
    async def _get_account_from_peer(
        self, block_hash: Hash32, address: Address, peer: LESPeer
    ) -> Account:
        key = keccak(address)
        ...
        try:
            ...
            raise BadLESResponse(
                "Peer %s returned an invalid proof for account %s at block %s"
                % (peer, encode_hex(address), encode_hex(block_hash))
            ) from exc
    ...
    async def coro_get_contract_code(
        ...
        try:
            ...
            raise NoEligiblePeers(
                "Our best peer does not have header %s" % block_hash
            ) from exc
    ...
    async def _get_contract_code_from_peer(
        self, block_hash: Hash32, address: Address, code_hash: Hash32, peer: LESPeer
    ) -> bytes:
        """
    ...
    async def _raise_for_empty_code(
        ...
        try:
            ...
            raise NoEligiblePeers(
                "Our best peer does not have the header %s" % block_hash
            )
        ...
        if head_number - header.block_number > MAX_REORG_DEPTH:
            ...
            if (
                await self.headerdb.coro_get_canonical_block_hash(header.block_number)
                == block_hash
            ):
                ...
                raise BadLESResponse(
                    ...
                    % (peer, encode_hex(code_hash), encode_hex(address))
                )
            else:
                # our header isn't canonical, so treat the empty response as missing data
                raise NoEligiblePeers(
                    "Our best peer does not have the non-canonical header %s"
                    % block_hash
                )
            ...
            raise NoEligiblePeers(
                "Our best peer's head does include header %s" % block_hash
            )
        else:
            ...
            raise NoEligiblePeers(
                ...
                % (peer, header, code_hash, head_number)
            )

    async def _get_block_header_by_hash(
        self, block_hash: Hash32, peer: LESPeer
    ) -> BlockHeader:
        """
        ...
        headers = await peer.requests.get_block_headers(
            block_hash, max_headers, skip=0, reverse=False
        )
        if not headers:
    ...
    async def _get_proof(
        ...
        request_id = peer.sub_proto.send_get_proof(
            block_hash, account_key, key, from_level
        )
        reply = await self._wait_for_reply(request_id)
        return reply["proof"]

    async def _retry_on_bad_response(
        self, make_request_to_peer: Callable[[LESPeer], Any]
    ) -> Any:
        """
        ...
        raise TimeoutError(
            "Could not complete peer request in %d attempts" % MAX_REQUEST_ATTEMPTS
        )
