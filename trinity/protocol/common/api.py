...
from trinity.protocol.eth.api import ETHV63API, ETHV64API, ETHV65API, ETHV66API
...
AnyETHLES = HasProtocol(ETHProtocolV66) | HasProtocol(ETHProtocolV65) | HasProtocol(
    ETHProtocolV64) | HasProtocol(ETHProtocolV63) | HasProtocol(
    LESProtocolV2) | HasProtocol(LESProtocolV1)

AnyETHLESAPI = Union[ETHV66API, ETHV65API, ETHV64API, ETHV63API, LESV1API, LESV2API]


def choose_eth_or_les_api(
        connection: ConnectionAPI) -> AnyETHLESAPI:

    if connection.has_protocol(ETHProtocolV66):
        return connection.get_logic(ETHV66API.name, ETHV66API)
    elif connection.has_protocol(ETHProtocolV65):
