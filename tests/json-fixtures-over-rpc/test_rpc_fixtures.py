...
from eth_utils.toolz import dissoc, identity, get_in

from eth_utils import add_0x_prefix, is_hex, is_integer, is_string
...
from trinity.rpc.format import empty_to_0x, remove_leading_zeros
...
RPC_TRANSACTION_REMAPPERS = {"data": "input", "gasLimit": "gas"}
...
def pytest_generate_tests(metafunc):
    generate_fixture_tests(
        ...
        filter_fn=filter_fixtures(
            fixtures_base_dir=BASE_FIXTURE_PATH, mark_fn=blockchain_fixture_mark_fn
        ),
    )
...
async def validate_account_attribute(
    *, fixture_key, rpc_method, rpc, state, addr, at_block
):
...
async def validate_account_state(rpc, state, addr, at_block):
    ...
    for key in state["storage"]:
        ...
        await assert_rpc_result(
            rpc, "eth_getStorageAt", [addr, position, at_block], expected_storage
        )
...
def validate_rpc_block_vs_fixture_header(block, header_fixture):
    ...
    actual_block = dissoc(block, "size", "totalDifficulty", "transactions", "uncles")
...
def validate_rpc_transaction_vs_fixture(transaction, fixture):
    ...
    actual_transaction = dissoc(transaction, "hash")
...
@pytest.mark.asyncio
async def test_rpc_against_fixtures(
    chain, event_bus, ipc_server, chain_fixture, fixture_data
):
    rpc = RPCServer(initialize_eth1_modules(MainnetFullChain(None), event_bus))

    setup_result, setup_error = await call_rpc(
        rpc, "evm_resetToGenesisFixture", [chain_fixture]
    )
    # We need to advance the event loop for modules to be able to pickup the new chain
    await asyncio.sleep(0)
    assert (
        setup_error is None and setup_result is True
    ), "cannot load chain for {0}".format(
        fixture_data
    )  # noqa: E501
    ...
    for block_fixture in chain_fixture["blocks"]:
        ...
        block_result, block_error = await call_rpc(
            rpc, "evm_applyBlockFixture", [block_fixture]
        )
        ...
        if should_be_good_block:
            ...
            await validate_block(
                rpc, block_fixture, block_fixture["blockHeader"]["hash"]
            )
        else:
    ...
    if chain_fixture.get("lastblockhash", None):
        for block_fixture in chain_fixture["blocks"]:
            if (
                get_in(["blockHeader", "hash"], block_fixture)
                == chain_fixture["lastblockhash"]
            ):
