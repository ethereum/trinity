...
DIR_GENESIS_JSON="mainnet"
FILE_GENESIS_JSON="genesis.json"

rm -rf $DIR_ALICE $DIR_BOB >/dev/null
mkdir -p $DIR_ALICE/$DIR_GENESIS_JSON $DIR_BOB/$DIR_GENESIS_JSON >/dev/null

# generate the genesis.json
$EXE_GEN_GENESIS > "$DIR_ALICE/$DIR_GENESIS_JSON/$FILE_GENESIS_JSON"
$EXE_GEN_GENESIS > "$DIR_BOB/$DIR_GENESIS_JSON/$FILE_GENESIS_JSON"
