...
from eth2.beacon.typing import Epoch, Gwei, SigningRoot, Slot, ValidatorIndex
...
class Context:
    ...
    @classmethod
    def at_time(
        cls, current_time: int, state: BeaconState, block: BaseBeaconBlock
    ) -> "Context":
        """
...
class Store:
    ...
    def get_ancestor(self, root: SigningRoot, slot: Slot) -> Optional[Hash32]:
        """
        ...
        if block.slot > slot:
            return self.get_ancestor(block.parent_root, slot)
    ...
    def get_latest_attesting_balance(self, root: SigningRoot) -> Gwei:
        ...
        return Gwei(
            sum(
                _balance_for_validator(state, i)
    ...
    def _should_update_justified_checkpoint(
        ...
        justified_ancestor = self.get_ancestor(
    ...
    def on_block(
        ...
        finalized_ancestor = self.get_ancestor(root, finalized_slot)
    ...
    def on_attestation(
        ...
        if target.epoch == current_epoch:
            pass
        elif target.epoch == previous_epoch:
            pass
        else:
            raise ValidationError(
        ...
        # NOTE: this invariant should hold based on how we handle
        # attestation importing in the chain but we will sanity check for now
        ...
        for i in indexed_attestation.attesting_indices:
            if (
                ...
                self._context.latest_messages[i] = msg
