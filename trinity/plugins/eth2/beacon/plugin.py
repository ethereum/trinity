...
from trinity.db.beacon.manager import create_db_consumer_manager
...
class BeaconNodePlugin(BaseIsolatedPlugin):
    ...
    def do_start(self) -> None:
        ...
        syncer = BeaconChainSyncer(chain_db, server.peer_pool, server.cancel_token)

        loop = asyncio.get_event_loop()
        asyncio.ensure_future(
            exit_with_endpoint_and_services(self.context.event_bus, server)
        )
