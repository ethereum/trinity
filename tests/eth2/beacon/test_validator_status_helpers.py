...
from eth2.beacon.helpers import get_delayed_activation_exit_epoch
...
# State update
#
@pytest.mark.parametrize(("is_genesis,"), [(True), (False)])
    ...
    state = filled_beacon_state.copy(
        validator_registry=tuple(
            mock_validator_record(
                pubkey=index.to_bytes(48, "little"), config=config, is_active=False
            )
...
def test_initiate_validator_exit(n_validators_state):
    ...
    result_state = initiate_validator_exit(state, index)
...
@pytest.mark.parametrize(
    ...
    [
        (10, 50, [4, 5, 6, 7], 100, 10),
        (10, 10, [4, 5, 6, 7], 100, 110),
        (10, 50, [4, 5, 6, 7], 100, 200),
    ],
)
def test_exit_validator(
    ...
    state = state.update_validator_registry(validator_index=index, validator=validator)
...
@pytest.mark.parametrize(("num_validators, committee"), [(10, [4, 5, 6, 7])])
def test_settle_penality_to_validator_and_whistleblower(
    ...
    state = n_validators_state
    validator_index = 5
    whistleblower_index = get_beacon_proposer_index(state, state.slot, committee_config)
...
def test_slash_validator(
    ...
    expected_state = exit_validator(
        state, index, slots_per_epoch, activation_exit_delay
    )
    ...
    validator = state.validator_registry[index].copy(
        slashed=False, withdrawable_epoch=current_epoch + latest_slashed_exit_length
    )
...
def test_prepare_validator_for_withdrawal(
    n_validators_state, slots_per_epoch, min_validator_withdrawability_delay
):
    state = n_validators_state
    index = 1
    result_state = prepare_validator_for_withdrawal(
        state, index, slots_per_epoch, min_validator_withdrawability_delay
    )
...
@pytest.mark.parametrize(
    ("slots_per_epoch", "state_slot", "validate_withdrawable_epoch", "success"),
    [(4, 8, 1, False), (4, 8, 2, False), (4, 7, 2, True), (4, 8, 3, True)],
)
def test_validate_withdrawable_epoch(
    slots_per_epoch, state_slot, validate_withdrawable_epoch, success
):
    if success:
        _validate_withdrawable_epoch(
