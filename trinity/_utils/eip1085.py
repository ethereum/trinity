...
from typing import Any, cast, Dict, Iterable, NamedTuple, Tuple
...
def get_eip1085_schema() -> Dict[str, Any]:
    ...
    if base_trinity_dir.name != "trinity":
        raise RuntimeError(
            f"Expected to be in root `trinity` module. Got: {str(base_trinity_dir)}"
        )
    ...
    if "homesteadForkBlock" in vm_config.keys():
        ...
        if "DAOForkBlock" in vm_config:
            ...
            HomesteadVM = BaseHomesteadVM.configure(
                support_dao_fork=True, _dao_fork_block_number=dao_fork_block_number
            )
...
def _filter_vm_config(vm_config: VMConfiguration) -> Iterable[VMFork]:
    for idx, (fork_block, vm_class) in enumerate(vm_config):
        if fork_block == 0:
            subsequent_fork_blocks = {
                block_num for block_num, _ in vm_config[idx + 1 :]
            }
...
ALL_VMS_BY_FORK = {vm_class.fork: vm_class for vm_class in ALL_VMS}
            ...
            if right_vm.fork in all_fork_names:
                if left_vm.fork in all_fork_names:
                    ...
                    raise ValidationError(
                        f"Invariant: Found {left_vm} in vm configuration"
                    )
...
def extract_vm_configuration(genesis_config: RawEIP1085Dict) -> VMConfiguration:
    ...
    """
    return _normalize_vm_config(
        _filter_vm_config(_extract_vm_config(genesis_config["params"]))
    )
...
def _normalize_genesis_state(
    genesis_state: Dict[HexAddress, RawAccountDetails]
) -> Iterable[Tuple[Address, Account]]:
...
def extract_genesis_data(raw_genesis_config: RawEIP1085Dict) -> GenesisData:
    ...
    return GenesisData(
        chain_id=chain_id, params=params, state=state, vm_configuration=vm_configuration
    )
