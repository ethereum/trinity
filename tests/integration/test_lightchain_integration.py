...
import pytest
import rlp
from eth_utils import decode_hex, encode_hex, to_text
...
async def test_lightchain_integration(
    ...
    peer_pool = LESPeerPool(privkey=ecies.generate_privkey(), context=context)
    ...
    await asyncio.sleep(
        0
    )  # Yield control to give the LightChainSyncer a chance to start
