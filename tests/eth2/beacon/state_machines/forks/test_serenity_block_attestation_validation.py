...
from eth2.beacon.committee_helpers import get_crosslink_committees_at_slot
...
from eth2.beacon.tools.builder.validator import create_mock_signed_attestation
...
def test_validate_attestation_source_epoch_and_root(
    ...
    if is_valid:
        validate_attestation_source_epoch_and_root(
            state, attestation_data, current_epoch, slots_per_epoch
        )
    else:
        with pytest.raises(ValidationError):
            validate_attestation_source_epoch_and_root(
...
@pytest.mark.parametrize(
    ...
    [
        (Crosslink(0, b"\x11" * 32), b"\x33" * 32, Crosslink(0, b"\x22" * 32), False),
        (Crosslink(0, b"\x33" * 32), b"\x33" * 32, Crosslink(0, b"\x11" * 32), False),
        (Crosslink(0, b"\x11" * 32), b"\x33" * 32, Crosslink(0, b"\x33" * 32), True),
        (Crosslink(0, b"\x33" * 32), b"\x22" * 32, Crosslink(0, b"\x33" * 32), True),
        (Crosslink(0, b"\x33" * 32), b"\x33" * 32, Crosslink(0, b"\x33" * 32), True),
    ],
)
def test_validate_attestation_latest_crosslink(
    sample_attestation_data_params,
    attestation_previous_crosslink,
    attestation_crosslink_data_root,
    state_latest_crosslink,
    ...
    if is_valid:
        validate_attestation_previous_crosslink_or_root(
            attestation_data, state_latest_crosslink, slots_per_epoch=slots_per_epoch
        )
    else:
        with pytest.raises(ValidationError):
            validate_attestation_previous_crosslink_or_root(
...
@pytest.mark.parametrize(
    ("attestation_crosslink_data_root," "is_valid,"),
    [(ZERO_HASH32, True), (b"\x22" * 32, False), (b"\x11" * 32, False)],
)
def test_validate_attestation_crosslink_data_root(
    sample_attestation_data_params, attestation_crosslink_data_root, is_valid
):
    attestation_data = AttestationData(**sample_attestation_data_params).copy(
        crosslink_data_root=attestation_crosslink_data_root
    )
    ...
    if is_valid:
        validate_attestation_crosslink_data_root(attestation_data)
    else:
        with pytest.raises(ValidationError):
            validate_attestation_crosslink_data_root(attestation_data)
...
def test_validate_attestation_aggregate_signature(
    ...
    crosslink_committee = get_crosslink_committees_at_slot(
        state=state, slot=slot, committee_config=committee_config
    )[0]
    ...
    attestation = create_mock_signed_attestation(
        state, attestation_data, committee, votes_count, keymap, slots_per_epoch
    )
    ...
    if is_valid:
        ...
        with pytest.raises(ValidationError):
            validate_attestation_aggregate_signature(
                state, attestation, committee_config
            )
