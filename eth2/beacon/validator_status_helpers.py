...
def activate_validator(
    ...
    validator = state.validator_registry[index].copy(
        ...
        if is_genesis
        else get_delayed_activation_exit_epoch(
...
def exit_validator(
    ...
    validator = validator.copy(exit_epoch=delayed_activation_exit_epoch)
    state = state.update_validator_registry(index, validator)
...
def _settle_penality_to_validator_and_whistleblower(
    *,
    ...
    current_epoch_penalization_index = (
        state.current_epoch(slots_per_epoch) % latest_slashed_exit_length
    )
    effective_balance = get_effective_balance(
        state.validator_balances, validator_index, max_deposit_amount
    )
...
def slash_validator(
    *,
    ...
    _validate_withdrawable_epoch(
        state.slot, validator.withdrawable_epoch, slots_per_epoch
    )
...
def _validate_withdrawable_epoch(
    state_slot: Slot, validator_withdrawable_epoch: Epoch, slots_per_epoch: int
) -> None:
    if state_slot >= get_epoch_start_slot(
        validator_withdrawable_epoch, slots_per_epoch
    ):
