...
from eth2.beacon.constants import EMPTY_SIGNATURE, GWEI_PER_ETH, FAR_FUTURE_EPOCH
...
from tests.eth2.beacon.helpers import get_pseudo_chain
...
def generate_mock_latest_historical_roots(
    ...
    latest_block_roots = [block.signing_root for block in blocks[:current_slot]] + [
...
def test_get_block_root(
    ...
    state = BeaconState(**sample_beacon_state_params).copy(
        slot=current_slot, latest_block_roots=latest_block_roots
    )

    if success:
        block_root = get_block_root(state, target_slot, slots_per_historical_root)
        assert block_root == blocks[target_slot].signing_root
    else:
        with pytest.raises(ValidationError):
            get_block_root(state, target_slot, slots_per_historical_root)
...
def test_get_state_root(
    ...
    state = BeaconState(**sample_beacon_state_params).copy(
        slot=current_slot, latest_state_roots=latest_state_roots
    )

    if success:
        state_root = get_state_root(state, target_slot, slots_per_historical_root)
        assert state_root == blocks[target_slot].signing_root
    else:
        with pytest.raises(ValidationError):
            get_state_root(state, target_slot, slots_per_historical_root)
...
@pytest.mark.parametrize(
    ("balance," "max_deposit_amount," "expected"),
    [
        (1 * GWEI_PER_ETH, 32 * GWEI_PER_ETH, 1 * GWEI_PER_ETH),
        (32 * GWEI_PER_ETH, 32 * GWEI_PER_ETH, 32 * GWEI_PER_ETH),
        (33 * GWEI_PER_ETH, 32 * GWEI_PER_ETH, 32 * GWEI_PER_ETH),
    ],
)
def test_get_effective_balance(
    balance, max_deposit_amount, expected, sample_validator_record_params
):
    balances = (balance,)
...
@pytest.mark.parametrize(
    ("validator_balances," "validator_indices," "max_deposit_amount," "expected"),
    [
        (tuple(), tuple(), 1 * GWEI_PER_ETH, 0),
        (
...
def test_get_total_balance(
    ...
    total_balance = get_total_balance(
        validator_balances, validator_indices, max_deposit_amount
    )
...
def test_get_fork_version(
    ...
    assert expected == get_fork_version(fork, current_epoch)
...
def test_get_domain(
    previous_version, current_version, epoch, current_epoch, domain_type, expected
):
    fork = Fork(
        previous_version=previous_version, current_version=current_version, epoch=epoch
    )
    assert expected == get_domain(
        fork=fork, epoch=current_epoch, domain_type=domain_type
    )


def test_is_double_vote(sample_attestation_data_params, slots_per_epoch):
    attestation_data_1_params = {**sample_attestation_data_params, "slot": 12345}
    attestation_data_1 = AttestationData(**attestation_data_1_params)

    attestation_data_2_params = {**sample_attestation_data_params, "slot": 12345}
    ...
    attestation_data_3_params = {**sample_attestation_data_params, "slot": 54321}
...
def test_is_surround_vote(
    ...
    assert (
        is_surround_vote(attestation_data_1, attestation_data_2, slots_per_epoch)
        == expected
    )
...
def test_generate_seed(
    ...
    def mock_get_randao_mix(state, epoch, slots_per_epoch, latest_randao_mixes_length):
        return hash_eth2(
            state.root
    ...
    def mock_get_active_index_root(
        state,
        epoch,
        slots_per_epoch,
        activation_exit_delay,
    ...
    monkeypatch.setattr(helpers, "get_active_index_root", mock_get_active_index_root)
    ...
    seed = generate_seed(state=state, epoch=epoch, committee_config=committee_config)
