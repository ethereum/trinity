...
from trinity.protocol.bcc.peer import BCCPeerFactory, BCCPeerPool
...
from tests.core.integration_test_helpers import async_passthrough
...
class FakeAsyncBeaconChainDB(BaseAsyncBeaconChainDB, BeaconChainDB):
    ...
    coro_get_canonical_block_root_by_slot = async_passthrough(
        "get_canonical_block_root_by_slot"
    )
...
async def _setup_alice_and_bob_factories(alice_chain_db, bob_chain_db):
    ...
    # Alice
    #
    alice_context = BeaconContext(chain_db=alice_chain_db, network_id=1)
    ...
    # Bob
    #
    bob_context = BeaconContext(chain_db=bob_chain_db, network_id=1)

    bob_factory = BCCPeerFactory(
        privkey=ecies.generate_privkey(), context=bob_context, token=cancel_token
    )
...
async def get_directly_linked_peers_without_handshake(alice_chain_db, bob_chain_db):
    alice_factory, bob_factory = await _setup_alice_and_bob_factories(
        alice_chain_db, bob_chain_db
    )

    return await _get_directly_linked_peers_without_handshake(
        alice_factory=alice_factory, bob_factory=bob_factory
    )
...
async def get_directly_linked_peers(request, event_loop, alice_chain_db, bob_chain_db):
    ...
    return await _get_directly_linked_peers(
        request, event_loop, alice_factory=alice_factory, bob_factory=bob_factory
    )


async def get_directly_linked_peers_in_peer_pools(
    request, event_loop, alice_chain_db, bob_chain_db
):
    alice, bob = await get_directly_linked_peers(
        request, event_loop, alice_chain_db=alice_chain_db, bob_chain_db=bob_chain_db
    )
