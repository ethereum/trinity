...
from p2p.abc import ServiceEventsAPI, LegacyServiceAPI
...
class BaseService(CancellableMixin, LegacyServiceAPI):
    # Use a WeakSet so that we don't have to bother updating it when tasks finish.
    _child_services: 'WeakSet[LegacyServiceAPI]'
    _tasks: 'WeakSet[asyncio.Future[Any]]'
    _finished_callbacks: List[Callable[[LegacyServiceAPI], None]]
    ...
    def add_finished_callback(self, finished_callback: Callable[[LegacyServiceAPI], None]) -> None:
        self._finished_callbacks.append(finished_callback)
    ...
    def run_child_service(self, child_service: LegacyServiceAPI) -> None:
        """
...
def service_timeout(timeout: int) -> Callable[..., Any]:
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def wrapped(service: LegacyServiceAPI, *args: Any, **kwargs: Any) -> Any:
...
TService = TypeVar('TService', bound=LegacyServiceAPI)
