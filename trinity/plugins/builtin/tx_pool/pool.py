from typing import cast, Callable, Iterable, List, FrozenSet, Type
import uuid
...
class TxPool(BaseService, PeerSubscriber):
    ...
    async def _handle_tx(self, peer: ETHPeer, txs: List[BaseTransactionFields]) -> None:
        ...
        async for receiving_peer in self._peer_pool:
            ...
            self.logger.debug2(
                "Sending %d transactions to %s", len(filtered_tx), receiving_peer
            )
    ...
    def _filter_tx_for_peer(
        self, peer: ETHPeer, txs: List[BaseTransactionFields]
    ) -> List[BaseTransactionFields]:
    ...
    def _add_txs_to_bloom(
        self, peer: ETHPeer, txs: Iterable[BaseTransactionFields]
    ) -> None:
        for val in txs:
