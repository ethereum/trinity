...
from eth_utils.toolz import assoc, curry
import ssz
...
from eth2.beacon.epoch_processing_helpers import get_base_reward, get_effective_balance
...
def test_ensure_update_eth1_vote_if_exists(
    sample_beacon_state_params, config, vote_offsets
):
    ...
    data_votes = tuple(
        Eth1DataVote(
            eth1_data=Eth1Data(
                deposit_root=_some_bytes(offset), block_hash=_some_bytes(offset)
            ),
...
def test_only_process_eth1_data_votes_per_period(sample_beacon_state_params, config):
    ...
    epochs_to_process_votes = [
        (epochs_per_voting_period * epoch) - 1
        for epoch in range(1, number_of_epochs_to_sample + 1)
    ]
...
#
@pytest.mark.parametrize(
    "total_balance," "current_epoch_boundary_attesting_balance," "expected,",
    (
...
def test_is_epoch_justifiable(
    ...
    with monkeypatch.context() as m:
        m.setattr(epoch_processing, "get_total_balance", mock_get_total_balance)
        m.setattr(
            epoch_processing,
        ...
        epoch_justifiable = _is_epoch_justifiable(
            sample_state, sample_state.current_epoch_attestations, current_epoch, config
        )
...
def test_get_finalized_epoch(
    justification_bitfield, previous_justified_epoch, current_justified_epoch, expected
):
    previous_epoch = 5
    finalized_epoch = 1
    assert (
...
def test_justification_without_mock(
    sample_beacon_state_params, slots_per_historical_root, config
):
...
@pytest.mark.parametrize(("genesis_slot,"), [(0)])
@pytest.mark.parametrize(
...
def test_process_justification(
    monkeypatch, config, genesis_state, states, genesis_epoch=0
):
    from eth2.beacon.state_machines.forks.serenity import epoch_processing
    ...
    for i in range(len(states) - 1):
        ...
        with monkeypatch.context() as m:
            m.setattr(
                epoch_processing, "_is_epoch_justifiable", mock_is_epoch_justifiable
            )
...
@pytest.mark.parametrize(
    ("n," "slots_per_epoch," "target_committee_size," "shard_count," "genesis_slot,"),
    [(90, 10, 9, 10, 0)],
)
@pytest.mark.parametrize(
    ("success_crosslink_in_previous_epoch," "success_crosslink_in_current_epoch,"),
...
def test_process_crosslinks(
    ...
    shard = 1
    previous_epoch_crosslink_data_root = hash_eth2(
        b"previous_epoch_crosslink_data_root"
    )
    ...
    state = n_validators_state.copy(
        slot=current_slot, latest_crosslinks=genesis_crosslinks
    )
        ...
        for committee, _shard in get_crosslink_committees_at_slot(
            ...
            if _shard == shard:
                ...
                if success_crosslink_in_previous_epoch:
                    attesting_validators = random.sample(
                        committee, (2 * len(committee) // 3 + 1)
                    )
                else:
                    attesting_validators = random.sample(
                        committee, (2 * len(committee) // 3 - 1)
    ...
    for slot_in_current_epoch in range(current_epoch_start_slot, next_epoch_start_slot):
        if len(current_epoch_attestations) > 0:
            break
        for committee, _shard in get_crosslink_committees_at_slot(
            state, slot_in_current_epoch, CommitteeConfig(config)
        ):
            if _shard == shard:
                # Sample validators attesting to this shard.
    ...
    assert (
        state.latest_crosslinks[shard].epoch == config.GENESIS_EPOCH
        and state.latest_crosslinks[shard].crosslink_data_root == ZERO_HASH32
    )
    ...
    if success_crosslink_in_current_epoch:
        ...
        assert (
            crosslink_record.epoch == current_epoch
            and crosslink_record.crosslink_data_root
            == attestation.data.crosslink_data_root
            and attestation.data.crosslink_data_root
            == current_epoch_crosslink_data_root
        )
        ...
        assert (
            ...
            == attestation.data.crosslink_data_root
            and attestation.data.crosslink_data_root
            == previous_epoch_crosslink_data_root
        )
    else:
        assert (
            crosslink_record.epoch == config.GENESIS_EPOCH
            and crosslink_record.crosslink_data_root == ZERO_HASH32
        )
...
def test_process_rewards_and_penalties_for_finality(
    ...
    def mock_get_beacon_proposer_index(
        state, slot, committee_config, registry_change=False
    ):
        mock_proposer_for_slot = {13: 12, 14: 5, 15: 1}
        return mock_proposer_for_slot[slot]

    monkeypatch.setattr(
        epoch_processing, "get_beacon_proposer_index", mock_get_beacon_proposer_index
    )

    validator_registry = n_validators_state.validator_registry
    for index in penalized_validator_indices:
        validator_record = validator_registry[index].copy(slashed=True)
        validator_registry = update_tuple_item(
            validator_registry, index, validator_record
        )
    state = n_validators_state.copy(
        slot=current_slot,
    ...
    previous_total_balance = (
        len(previous_epoch_active_validator_indices) * effective_balance
    )

    attestation_slot = current_slot - slots_per_epoch
    inclusion_infos = {
        index: InclusionInfo(
            attestation_slot + inclusion_distances[index], attestation_slot
        )
        for index in previous_epoch_attester_indices
    }

    effective_balances = {
        index: effective_balance for index in range(len(state.validator_registry))
    }

    base_rewards = {
        index: base_reward for index in range(len(state.validator_registry))
    }
            ...
            PendingAttestation(**sample_pending_attestation_record_params).copy(
                ...
                data=AttestationData(**sample_attestation_data_params).copy(
                    ...
                    target_root=get_block_root(
                        state, prev_epoch_start_slot, config.SLOTS_PER_HISTORICAL_ROOT
                    ),
...
def test_process_rewards_and_penalties_for_crosslinks(
    ...
    for i in range(slots_per_epoch):
        ...
        each_slot_attestion_validators_list.append(
            crosslink_data_root_attesting_validators
        )
        participants_bitfield = get_empty_bitfield(target_committee_size)
        ...
        previous_epoch_attestations.append(
            PendingAttestation(**sample_pending_attestation_record_params).copy(
                ...
                data=AttestationData(**sample_attestation_data_params).copy(
                    ...
                    shard=shard,
                    previous_crosslink=Crosslink(
                        epoch=config.GENESIS_EPOCH, crosslink_data_root=ZERO_HASH32
                    ),
                ),
    ...
    active_validators = set([i for i in range(len(state.validator_registry))])
    ...
    rewards_received, penalties_received = _process_rewards_and_penalties_for_crosslinks(
        state, config, effective_balances, base_rewards
    )

    expected_rewards_received = {
        index: 0 for index in range(len(state.validator_registry))
    }
    for i in range(slots_per_epoch):
...
def test_process_ejections(genesis_state, config, activation_exit_delay):
    ...
    state = state.update_validator_balance(
        validator_index=ejecting_validator_index, balance=config.EJECTION_BALANCE - 1
    )
...
def test_check_if_update_validator_registry(
    ...
    if has_crosslink:
        crosslink = Crosslink(epoch=crosslink_epoch, crosslink_data_root=ZERO_HASH32)
        latest_crosslinks = state.latest_crosslinks
        for shard in range(config.SHARD_COUNT):
            latest_crosslinks = update_tuple_item(latest_crosslinks, shard, crosslink)
...
@pytest.mark.parametrize(
    ("n", "slots_per_epoch", "target_committee_size", "shard_count"), [(10, 10, 9, 10)]
)
def test_update_validator_registry(n, n_validators_state, config, slots_per_epoch):
    validator_registry = list(n_validators_state.validator_registry)
    activating_index = n
    exiting_index = 0

    activating_validator = Validator.create_pending_validator(
        pubkey=b"\x10" * 48, withdrawal_credentials=b"\x11" * 32
    )

    exiting_validator = n_validators_state.validator_registry[exiting_index].copy(
        exit_epoch=FAR_FUTURE_EPOCH, initiated_exit=True
    )
    ...
    state = n_validators_state.copy(
        validator_registry=validator_registry,
        validator_balances=n_validators_state.validator_balances
    ...
    assert (
        state.validator_registry[activating_index].activation_epoch
...
def test_process_validator_registry(
    ...
    monkeypatch.setattr("eth2.beacon.helpers.generate_seed", mock_generate_seed)
    ...
    assert (
        result_state.previous_shuffling_start_shard
        == state.current_shuffling_start_shard
    )
    assert result_state.previous_shuffling_seed == state.current_shuffling_seed

    if need_to_update:
        assert result_state.current_shuffling_epoch == slot_to_epoch(
            state_slot, slots_per_epoch
        )
        assert result_state.current_shuffling_seed == new_seed
        # TODO: Add test for validator registry updates
    else:
        assert result_state.current_shuffling_epoch == expected_current_shuffling_epoch
        # state.current_shuffling_start_shard is left unchanged.
        assert (
            result_state.current_shuffling_start_shard
            == state.current_shuffling_start_shard
        )
...
def test_compute_total_penalties(
    genesis_state,
    config,
    slots_per_epoch,
    current_epoch,
    ...
    total_penalties = _compute_total_penalties(state, config, current_epoch)
...
@pytest.mark.parametrize(
    ("total_penalties", "total_balance", "min_penalty_quotient", "expected_penalty"),
    [
        (
            10 ** 9,  # 1 ETH
            (32 * 10 ** 9 * 10),
...
def test_compute_individual_penalty(
    genesis_state,
    config,
    slots_per_epoch,
    current_epoch,
...
@pytest.mark.parametrize(
    ...
    [(10, 4, 8, 8, 8, (2 * 10 ** 9, 10 ** 9) + (0,) * 6, 32 * 10 ** 9 // 2 ** 5)],
)
def test_process_slashings(
    ...
    validator = state.validator_registry[slashing_validator_index].copy(
        slashed=True, withdrawable_epoch=current_epoch + latest_slashed_exit_length // 2
    )
...
def test_process_exit_queue_eligible(
    ...
    state = state.update_validator_registry(
        ...
        state.validator_registry[validator_index].copy(
            withdrawable_epoch=withdrawable_epoch, exit_epoch=exit_epoch
        ),
    )
...
@pytest.mark.parametrize(
    ("max_exit_dequeues_per_epoch", "num_eligible_validators", "validator_exit_epochs"),
    [
...
def test_process_exit_queue(
    ...
    for i in range(num_eligible_validators):
        state = state.update_validator_registry(
            i, state.validator_registry[i].copy(exit_epoch=validator_exit_epochs[i])
        )
    ...
    sorted_indices = sorted(
        range(num_eligible_validators), key=lambda i: validator_exit_epochs[i]
    )
    filtered_indices = sorted_indices[
        : min(max_exit_dequeues_per_epoch, num_eligible_validators)
    ]
...
#
@pytest.mark.parametrize(
    ("slots_per_epoch," "latest_active_index_roots_length," "state_slot,"),
    [(4, 16, 4), (4, 16, 64)],
)
def test_update_latest_active_index_roots(
    genesis_state,
    committee_config,
    state_slot,
    slots_per_epoch,
    ...
    index_root = ssz.hash_tree_root(
        get_active_validator_indices(
            state.validator_registry, slot_to_epoch(state.slot, slots_per_epoch)
        ),
    ...
    assert (
        result_state.latest_active_index_roots[
...
@pytest.mark.parametrize(("num_validators," "slots_per_epoch"), [(10, 4)])
def test_process_final_updates(genesis_state, config, sample_attestation_params):
    current_slot = 10
    state = genesis_state.copy(slot=current_slot)
    current_index = (
        state.next_epoch(config.SLOTS_PER_EPOCH) % config.LATEST_SLASHED_EXIT_LENGTH
    )
    previous_index = (
        state.current_epoch(config.SLOTS_PER_EPOCH) % config.LATEST_SLASHED_EXIT_LENGTH
    )
    ...
    previous_epoch_attestations = [
        attestation.copy(
            data=attestation.data.copy(slot=previous_epoch_attestation_slot)
        )
        for _ in range(num_previous_epoch_attestations)
    ]
    num_current_epoch_attestations = 3
    current_epoch_attestations = [
        attestation.copy(data=attestation.data.copy(slot=current_slot))
    ...
    latest_slashed_balances = update_tuple_item(
        state.latest_slashed_balances, previous_index, slashed_balance_of_previous_epoch
    )
    state = state.copy(
        latest_slashed_balances=latest_slashed_balances,
        previous_epoch_attestations=previous_epoch_attestations,
    ...
    assert (
        ...
        result_state.latest_randao_mixes[current_index]
        == get_randao_mix(
            state=state,
