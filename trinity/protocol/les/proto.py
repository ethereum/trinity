...
class LESProtocol(Protocol):
    ...
    def send_get_block_bodies(
        self, block_hashes: List[bytes], request_id: int = None
    ) -> int:
        ...
        header, body = GetBlockBodies(self.cmd_id_offset, self.snappy_support).encode(
            data
        )
    ...
    def send_block_headers(
        ...
        header, body = BlockHeaders(self.cmd_id_offset, self.snappy_support).encode(
            data
        )
        self.send(header, body)
    ...
    def send_get_contract_code(
        self, block_hash: bytes, key: bytes, request_id: int = None
    ) -> int:
        if request_id is None:
            request_id = gen_request_id()
        data = {
        ...
        header, body = GetContractCodes(self.cmd_id_offset, self.snappy_support).encode(
            data
        )
        self.send(header, body)
