...
import sys
from typing import Any, cast, Dict, Tuple, TYPE_CHECKING, Callable

from eth_utils.toolz import dissoc

from eth.tools.logging import ExtendedDebugLogger

from trinity._utils.shellart import bold_red, bold_yellow
...
def setup_trinity_stderr_logging(
    level: int = None,
...
def setup_trinity_file_and_queue_logging(
    ...
    listener = QueueListener(
        log_queue, handler_stream, handler_file, respect_handler_level=True
    )
...
def with_queued_logging(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def inner(*args: Any, **kwargs: Any) -> Any:
        try:
            ...
            raise KeyError(
                f"The `log_queue` argument is required when calling `{fn.__name__}`"
            )
        else:
