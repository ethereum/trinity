...
from eth2.beacon.helpers import slot_to_epoch, get_temporary_block_header
from eth2.beacon.typing import Epoch, Gwei, Shard, Slot, Timestamp, ValidatorIndex
...
class BeaconState(ssz.Serializable):
    ...
    def update_validator_registry(
        self, validator_index: ValidatorIndex, validator: Validator
    ) -> "BeaconState":
        """
        ...
        updated_state = self.copy(validator_registry=tuple(validator_registry))
        return updated_state

    def update_validator_balance(
        ...
        updated_state = self.copy(validator_balances=tuple(validator_balances))
        return updated_state

    def update_validator(
        self, validator_index: ValidatorIndex, validator: Validator, balance: Gwei
    ) -> "BeaconState":
        """
