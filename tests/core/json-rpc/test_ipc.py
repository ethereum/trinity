...
from eth_utils import decode_hex, function_signature_to_4byte_selector, to_bytes, to_hex

from trinity.nodes.events import NetworkIdRequest, NetworkIdResponse
...
def build_request(method, params=None):
    if params is None:
        params = []
    request = {"jsonrpc": "2.0", "id": 3, "method": method, "params": params}
...
async def get_ipc_response(jsonrpc_ipc_pipe_path, request_msg, event_loop):

    assert wait_for(
        jsonrpc_ipc_pipe_path
    ), "IPC server did not successfully start with IPC file"

    reader, writer = await asyncio.open_unix_connection(
        str(jsonrpc_ipc_pipe_path), loop=event_loop
    )
    ...
    while not can_decode_json(result_bytes):
        result_bytes += await asyncio.tasks.wait_for(
            reader.readuntil(b"}"), 0.25, loop=event_loop
        )
...
@pytest.mark.parametrize(
    ...
    (
        (b"{}", {"error": "Invalid Request: empty"}),
        (
            build_request("notamethod"),
            {"error": "Invalid RPC method: 'notamethod'", "id": 3, "jsonrpc": "2.0"},
        ),
        (build_request("eth_accounts"), {"result": [], "id": 3, "jsonrpc": "2.0"}),
        (build_request("eth_mining"), {"result": False, "id": 3, "jsonrpc": "2.0"}),
        (
        ...
        (build_request("eth_hashrate"), {"result": "0x0", "id": 3, "jsonrpc": "2.0"}),
        (
        ...
        (build_request("net_listening"), {"result": True, "id": 3, "jsonrpc": "2.0"}),
    ),
)
async def test_ipc_requests(
    jsonrpc_ipc_pipe_path, request_msg, expected, event_loop, ipc_server
):
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            build_request(
                "eth_estimateGas", params=[{"to": "0x" + "00" * 20}, "latest"]
            ),
        ...
        (
            ...
            build_request(
                "eth_estimateGas",
                params=[{"to": "0x" + "00" * 20, "data": "0x01"}, "latest"],
            ),
        ...
        (
            ...
            build_request(
                "eth_estimateGas", params=[{"data": "0x3838533838f3"}, "latest"]
            ),
        ...
        (
            ...
            build_request("eth_estimateGas", params=[{"gas_price": "0x0"}, "latest"]),
            {
        ...
        (
            ...
            build_request("eth_estimateGas", params=[{"nonce": "0x01"}, "latest"]),
            {
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "doRevert()",
            0,
            {"error": "Invalid opcode 0xfd @ 415", "id": 3, "jsonrpc": "2.0"},
        ),
        (
            "useLotsOfGas()",
            0,
...
@pytest.mark.parametrize(
    ...
    ids=["net_peerCount_1", "net_peerCount_0"],
)
...
@pytest.mark.parametrize(
    ...
    ids=["eth_syncing_F", "eth_syncing_T"],
)
    ...
    (
        (
            build_request("admin_addPeer", ["enode://none@[::]:30303"]),
            {
                "jsonrpc": "2.0",
                "id": 3,
                "error": "public key must be a 128-character hex string",
            },
        ),
        (
            build_request("admin_addPeer", [f"enode://{GOOD_KEY}@[::]:30303"]),
            {
                "jsonrpc": "2.0",
                "id": 3,
                "error": "A concrete IP address must be specified",
            },
        ),
    ),
...
async def test_admin_addPeer_error_messages(
    jsonrpc_ipc_pipe_path, request_msg, event_loop, expected, ipc_server
):
...
async def test_admin_addPeer_fires_message(
    ...
    result = await get_ipc_response(jsonrpc_ipc_pipe_path, request, event_loop)
