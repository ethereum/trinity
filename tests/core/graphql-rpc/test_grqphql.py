...
def test_chain(chain, funded_address, funded_address_private_key):
    import uuid
    from eth._utils.address import force_bytes_to_address
    # vm = chain.get_vm()
    # state = vm.state
    tx = chain.create_unsigned_transaction(nonce=0,gas_price=1,gas=3138525,data=uuid.uuid4().bytes,to=force_bytes_to_address(b'\x10\x10'),value=1,).as_signed_transaction(funded_address_private_key)
    # header = chain.ensure_header()
    ...
    #
    # transactions = base_block.transactions + (tx,)
    # receipts = base_block.get_receipts(chain.chaindb) + (receipt,)
    #
