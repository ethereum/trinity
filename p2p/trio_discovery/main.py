...
from p2p.trio_utils import gather
...
class DiscoveryProtocol:
    ...
    logger: ExtendedDebugLogger = cast(ExtendedDebugLogger,
                                       logging.getLogger("p2p.discovery.DiscoveryProtocol[trio]"))
    use_v5 = False
    ...
    def __init__(self,
                 ...
                 nursery: trio_typing.Nursery) -> None:
    ...
    def check_ping_pong_lock(self, node: kademlia.Node) -> bool:
        if self.ping_callbacks.locked(node):
            return True
        elif self.pong_callbacks.locked(node):
            return True
        else:
            return False

    def is_find_locked(self, node: kademlia.Node) -> bool:
        return self.neighbours_callbacks.locked(node)
    ...
    def update_routing_table(self, node: kademlia.Node) -> None:
        ...
        if eviction_candidate:
            ...
            # replacement cache.
            self._nursery.start_soon(self.bond, eviction_candidate)
    ...
    async def _bond_with_return_over_channel(self,
                                             index: int,
                                             node: kademlia.Node,
                                             channel: trio.abc.SendChannel[Tuple[int, bool]]) -> None:
        did_bond = await self.bond(node)
        await channel.send((index, did_bond))

    async def bond_many(self, *nodes: kademlia.Node) -> Tuple[kademlia.Node, ...]:
        """
        Attempts to bond to all of the provided nodes, returning the nodes that
        successfully bonded.
        """
        (
            send_channel,
            receive_channel,
        ) = trio.open_memory_channel(len(nodes))

        async with trio.open_nursery() as nursery:
            for (index, candidate) in enumerate(nodes):
                nursery.start_soon(self._bond_with_return_over_channel, index, candidate, send_channel)
        bonded_nodes = tuple([
            node
            async for (did_bond, node)
            in receive_channel
            if did_bond
        ])
    ...
    async def wait_ping(self, remote: kademlia.Node) -> bool:
        ...
        async with self.ping_callbacks.acquire(remote, event.set):
            with trio.timeout_after(kademlia.k_request_timeout) as cancel_scope:
                await event.wait()
    ...
    async def _wait_pong(
            self, remote: kademlia.Node, token: Hash32, event: trio.Event,
    ...
    async def lookup(self, node_id: int) -> Tuple[kademlia.Node, ...]:
        ...
        """
        lookup = Lookup(node_id=node_id, discovery=self)
        closest = await lookup.lookup()
        self.logger.debug(
    ...
    async def bootstrap(self) -> None:
        ...
        try:
            coros_and_args = tuple(
                (self.bond, n)
                for n
                in self.bootstrap_nodes
                if not self.check_ping_pong_lock(n)
            )
            bonded = await gather(*coros_and_args)
            if not any(bonded):
    ...
    def receive(self, address: kademlia.Address, message: bytes) -> None:
        ...
        if trio.current_time() > expiration:
...
class Lookup:
    logger = logging.getLogger('p2p.trio_discovery.Lookup')

    _asked: Set[kademlia.Node]
    _seen: Set[kademlia.Node]

    def __init__(self, node_id: int, discovery: DiscoveryProtocol) -> None:
        self._discovery = discovery
        self.node_id = node_id
        self._nodes_asked = set()
        self._nodes_seen = set()

    async def _find_node(self,
                         node_id: int,  # TODO: use a `NewType` for `NodeId`.
                         remote: kademlia.Node,
                         ) -> Tuple[kademlia.Node, ...]:
        # check for cancellation so we don't send un-necessary find_node
        # requests
        await trio.hazmat.checkpoint()

        # send the find node request
        self._discovery._send_find_node(remote, node_id)

        # wait for the neighbors response for the remote
        candidates = await self._discovery.wait_neighbours(remote)

        if not candidates:
            self.logger.debug("got no candidates from %s, returning", remote)
            return ()

        all_candidates = tuple(c for c in candidates if c not in self._seen)
        candidates = tuple(
            c for c in all_candidates
            if not self._discovery.check_ping_pong_lock(c)
        )
        ...
        bonded = await self._discovery.bond_many(*candidates)
        self.logger.debug2("bonded with %s candidates", len(bonded))
        return bonded

    async def _find_many(self, node_id: int, *nodes: kademlia.Node) -> Tuple[kademlia.Node]:
        return results

    def _exclude_if_asked(self,
                          node_id: int,
                          nodes: Iterable[kademlia.Node]) -> Tuple[kademlia.Node, ...]:
        nodes_to_ask = set(nodes).difference(self._asked)
        return kademlia.sort_by_distance(nodes_to_ask, node_id)[:kademlia.k_find_concurrency]

    async def lookup(self) -> Tuple[kademlia.Node, ...]:
        """Lookup performs a network search for nodes close to the given target.

        It approaches the target by querying nodes that are closer to it on each iteration.  The
        given target does not need to be an actual node identifier.
        """
        closest = self._discovery.routing.neighbours(node_id)
        self.logger.debug("starting lookup; initial neighbours: %s", closest)
        nodes_to_ask = self._exclude_if_asked(node_id, closest)
        while nodes_to_ask:
            await trio.hazmat.checkpoint()
            self.logger.debug2("node lookup; querying %s", nodes_to_ask)
            self._asked.update(nodes_to_ask)

            coros_and_args = tuple(
                (self._find_node, (node_id, node))
                for node in nodes
                if not self._discovery.is_find_locked(node)
            )
            results = await gather(*coros_and_args)
...
class PreferredNodeDiscoveryProtocol(DiscoveryProtocol):
    ...
    def _get_eligible_preferred_nodes(self) -> Iterator[kademlia.Node]:
        ...
        for node in self.preferred_nodes:
            ...
            if trio.current_time() - last_used > self.preferred_node_recycle_time:
                yield node
    ...
    def get_random_bootnode(self) -> Iterator[kademlia.Node]:
        ...
        try:
            ...
            self._preferred_node_tracker[node] = trio.current_time()
            yield node
...
class DiscoveryByTopicProtocol(DiscoveryProtocol):
    ...
    async def lookup_random(self) -> Tuple[kademlia.Node, ...]:
        ...
        coros_and_args = tuple(
            (self.wait_topic_nodes, n, echo)
            for n, echo in expected_echoes
        )
        replies = await gather(*coros_and_args)
...
class DiscoveryService(BaseService):
    ...
    async def maybe_lookup_random_node(self) -> None:
        if self._last_lookup + self._lookup_interval > trio.current_time():
            return
        ...
        async with self._lookup_running:
            ...
            try:
                ...
                self._last_lookup = trio.current_time()
...
class TopicTable:
    ...
    def add_node(self, node: kademlia.Node, topic: bytes) -> None:
        ...
        entries[node] = trio.current_time() + self.registration_lifetime
...
def _get_msg_expiration() -> int:
    return int(trio.current_time() + EXPIRATION)
...
class CallbackLock:
    ...
    def is_expired(self) -> bool:
        return trio.current_time() - self.created_at > self.timeout
