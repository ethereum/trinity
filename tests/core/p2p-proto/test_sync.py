...
from trinity.sync.common.chain import SimpleBlockImporter
...
async def test_fast_syncer(request, event_loop, chaindb_fresh, chaindb_20):
    ...
    server_request_handler = ETHRequestServer(
        FakeAsyncChainDB(chaindb_20.db), server_peer_pool
    )
    ...
    class HeaderSyncer_OnlyOne:
        ...
        async def new_sync_headers(self, max_batch_size):
            async for headers in self._real_syncer.new_sync_headers(1):
                ...
                raise Exception(
                    "This should always get cancelled quickly, say within 1s"
                )
...
async def test_light_syncer(request, event_loop, chaindb_fresh, chaindb_20):
    ...
    server_request_handler = LightRequestServer(
        FakeAsyncHeaderDB(chaindb_20.db), server_peer_pool
    )
    asyncio.ensure_future(server_request_handler.run())
    server_peer.logger.info("%s is serving 20 blocks", server_peer)
    client_peer.logger.info("%s is syncing up 20", client_peer)
