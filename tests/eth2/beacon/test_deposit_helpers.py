...
def create_mock_deposit(
    ...
    state = state.copy(latest_eth1_data=state.latest_eth1_data.copy(deposit_root=root))

    deposit = Deposit(proof=proof, index=validator_index, deposit_data=deposit_data)

    return state, deposit


def test_add_pending_validator(
    ...
    amount = 5566
    state = add_pending_validator(state, validator, amount)

    assert state.validator_registry[-1] == validator


@pytest.mark.parametrize(("deposit_index", "success"), [(0, True), (1, False)])
def test_validate_deposit_order(
    config, sample_beacon_state_params, keymap, pubkeys, deposit_index, success
):
    validator_index = 0
...
def test_process_deposit(
    config, deposit_contract_tree_depth, sample_beacon_state_params, keymap, pubkeys
):
    validator_index = 0
    withdrawal_credentials = b"\x34" * 32
    state, deposit = create_mock_deposit(
