...
class BCCProtocol(HasExtendedDebugLogger, Protocol):
    name = "bcc"
    version = 0
    _commands = (Status, GetBeaconBlocks, BeaconBlocks, Attestations, NewBeaconBlock)
    cmd_length = 5
    ...
    def send_get_blocks(
        self, block_slot_or_root: Union[Slot, Hash32], max_blocks: int, request_id: int
    ) -> None:
    ...
    def send_attestation_records(self, attestations: Tuple[Attestation, ...]) -> None:
        ...
        header, body = cmd.encode(
            tuple(ssz.encode(attestation) for attestation in attestations)
        )
        self.send(header, body)
