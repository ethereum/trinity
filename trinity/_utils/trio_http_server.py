...
class TrioHTTPWrapper:
    logger = logging.getLogger("trinity._utils.trio_http_server.TrioHTTPWrapper")
...
# out.
@curry
async def http_serve(router, stream):
...
async def send_simple_response(
    wrapper, status_code, content_type, body, response_headers=None
):
...
def make_router(handlers):
    def _router(target, method):
        if target.path not in handlers:
            raise ResourceNotFoundException()
        handlers_for_path = handlers[target.path]
...
async def send_json_response(router, wrapper, request):
    ...
    response_json = await handler(target, method, headers, body.decode("ascii"))
    response_body_unicode = json.dumps(response_json, indent=4, separators=(",", ": "))
...
def _echo_router(target, method):
    async def _handler(target, method, headers, body):
        response_json = {
            "method": method,
            "target": target,
            "headers": headers,
