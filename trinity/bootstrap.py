...
def main_entry(
    ...
    sub_configs: Iterable[Type[BaseAppConfig]],
) -> None:

    main_endpoint = TrinityMainEventBusEndpoint()

    plugin_manager = setup_plugins(MainAndIsolatedProcessScope(main_endpoint), plugins)
    ...
    min_configured_log_level = min(
        stderr_logger.level, file_logger.level, *(args.log_levels or {}).values()
    )
...
def setup_plugins(
    scope: BaseManagerProcessScope, plugins: Iterable[BasePlugin]
) -> PluginManager:
...
def kill_trinity_gracefully(
    ...
    remove_dangling_ipc_files(
        logger, trinity_config.ipc_dir, except_file=main_endpoint.ipc_path
    )
