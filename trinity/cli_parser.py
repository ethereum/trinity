...
logging_parser.add_argument(
    ...
    dest="log_levels",
    metavar="LEVEL",
    help=("Configure the logging level. " + log_level_formatted_string()),
)
logging_parser.add_argument(
    "--stderr-log-level",
    dest="stderr_log_level",
    help=("Configure the logging level for the stderr logging."),
)
logging_parser.add_argument(
    "--file-log-level",
    dest="file_log_level",
    help=("Configure the logging level for file-based logging."),
)
...
network_parser.add_argument(
    "--max-peers", help=("Maximum number of network peers"), type=int
)
...
class EIP1085GenesisLoader(argparse.Action):
    def __call__(
        ...
        if not genesis_file_path.exists():
            raise argparse.ArgumentError(self, f"No genesis file found at: `{value}`")

        try:
            genesis_config = json.load(genesis_file_path.open())
        except json.JSONDecodeError:
            raise argparse.ArgumentError(
                self, f"The genesis file at `{value}` is not valid json"
            )
...
chain_parser.add_argument(
    "--genesis",
    help=("File containing a custom genesis block header"),
    action=EIP1085GenesisLoader,
)
chain_parser.add_argument(
    "--data-dir", help=("The directory where chain data is stored")
)
chain_parser.add_argument(
    "--nodekey",
    help=(
...
debug_parser.add_argument(
    "--profile", action="store_true", help=("Enables profiling via cProfile.")
)
