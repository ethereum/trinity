...
def validate_transaction_gas_estimation_dict(
    transaction_dict: Dict[str, Any], vm: BaseVM
) -> None:
    ...
    spec_keys = set(
        RENAMED_KEYS.get(field_name, field_name) for field_name in allowed_keys
    )
    ...
    if superfluous_keys:
        raise ValueError(
            ...
            % (list(sorted(superfluous_keys)), list(sorted(spec_keys)))
        )


def validate_transaction_call_dict(
    transaction_dict: Dict[str, Any], vm: BaseVM
) -> None:
    ...
    if not is_address(transaction_dict.get("to", None)):
        raise ValueError(
            "The 'to' field must be supplied when getting the result of a transaction"
        )
