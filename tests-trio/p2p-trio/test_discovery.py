...
from eth_enr import ENR, UnsignedENR, V4IdentityScheme
...
async def test_local_enr_on_startup(manually_driven_discovery):
    ...
    our_enr = discovery.enr_db.get_enr(discovery.this_node.id)
...
async def test_get_peer_candidates(manually_driven_discovery, monkeypatch):
    ...
    discovery = manually_driven_discovery
    for node in nodes:
        discovery.enr_db.set_enr(node.enr)
...
async def test_bond(nursery, monkeypatch):
    ...
    node2 = NodeFactory()
    discovery.enr_db.set_enr(node2.enr)
...
async def test_fetch_enrs(nursery, manually_driven_discovery_pair):
    ...
    alice.enr_db.set_enr(bob.this_node.enr)
    ...
    with trio.fail_after(1):
        ...
        while True:
            ...
            try:
                bob_enr = alice.enr_db.get_enr(bob.this_node.id)
            except KeyError:
                continue
            else:
                break
