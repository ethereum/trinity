...
from typing import cast, Any, Dict, Iterable, Optional, Tuple, Union
...
from trinity.constants import MAINNET_NETWORK_ID, ROPSTEN_NETWORK_ID, SYNC_LIGHT


DEFAULT_DATA_DIRS = {ROPSTEN_NETWORK_ID: "ropsten", MAINNET_NETWORK_ID: "mainnet"}
...
def get_nodekey_path(data_dir: Path) -> Path:
    ...
    """
    return Path(os.environ.get("TRINITY_NODEKEY", str(data_dir / NODEKEY_FILENAME)))
...
def get_database_socket_path(data_dir: Path) -> Path:
    ...
    """
    return Path(
        os.environ.get("TRINITY_DATABASE_IPC", data_dir / DATABASE_SOCKET_FILENAME)
    )
...
def get_jsonrpc_socket_path(data_dir: Path) -> Path:
    ...
    """
    return Path(
        os.environ.get("TRINITY_JSONRPC_IPC", data_dir / JSONRPC_SOCKET_FILENAME)
    )
...
def construct_trinity_config_params(args: argparse.Namespace) -> TrinityConfigParams:
    return cast(TrinityConfigParams, dict(_construct_trinity_config_params(args)))


def _construct_trinity_config_params(
    args: argparse.Namespace
) -> Iterable[Tuple[str, Union[int, str, bytes, Tuple[str, ...]]]]:
    """
    ...
    if args.genesis is not None:
        if args.data_dir is None:
            raise ValueError(
                "When providing a custom genesis, must also provide a data-dir"
            )
