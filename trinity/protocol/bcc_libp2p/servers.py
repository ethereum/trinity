...
class OrphanBlockPool:
    ...
    def __contains__(self, block_or_block_root: Union[BaseBeaconBlock, HashTreeRoot]) -> bool:
        ...
        if isinstance(block_or_block_root, BaseBeaconBlock):
            ...
            raise TypeError("`block_or_block_root` should be `BaseBeaconBlock` or `HashTreeRoot`")
        try:
            self.get(block_root)
            return True
    ...
    def get(self, block_root: HashTreeRoot) -> BaseBeaconBlock:
    ...
    def pop_children(self, block_root: HashTreeRoot) -> Tuple[BaseBeaconBlock, ...]:
        children = tuple(
            orphan_block
...
class BCCReceiveServer(BaseService):
    ...
    def _try_import_orphan_blocks(self, parent_root: HashTreeRoot) -> None:
        """
        ...
        """
        imported_roots: List[HashTreeRoot] = []
    ...
    def _is_block_root_in_orphan_block_pool(self, block_root: HashTreeRoot) -> bool:
        return block_root in self.orphan_block_pool

    def _is_block_root_in_db(self, block_root: HashTreeRoot) -> bool:
        try:
    ...
    def _is_block_root_seen(self, block_root: HashTreeRoot) -> bool:
