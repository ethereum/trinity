...
from eth2.beacon.helpers import get_domain, slot_to_epoch
...
def _generate_some_indices(data, max_value_for_list):
    ...
    """
    return data.draw(st.lists(st.integers(min_value=0, max_value=max_value_for_list)))
...
def test_generate_aggregate_pubkeys(
    activated_genesis_validators, sample_slashable_attestation_params, data
):
    max_value_for_list = len(activated_genesis_validators) - 1
    (validator_indices, some_index) = _list_and_index(
        data, elements=st.integers(min_value=0, max_value=max_value_for_list)
    )
    ...
    keys = generate_aggregate_pubkeys_from_indices(
        activated_genesis_validators, *slashable_attestation.custody_bit_indices
    )
    assert len(keys) == 2
    ...
    poc_1_keys = get_pubkey_for_indices(
        activated_genesis_validators, custody_bit_1_indices
    )
...
def _get_indices_and_signatures(num_validators, message_hash, privkeys, fork, epoch):
    ...
    domain = get_domain(fork=fork, epoch=epoch, domain_type=domain_type)
...
def _correct_slashable_attestation_params(
    slots_per_epoch, num_validators, params, message_hashes, privkeys, fork
):
...
def _corrupt_validator_indices_max(max_indices_per_slashable_vote, params):
    corrupt_validator_indices = [i for i in range(max_indices_per_slashable_vote + 1)]
    return assoc(params, "validator_indices", corrupt_validator_indices)
...
@pytest.mark.parametrize(("num_validators",), [(40,)])
def test_verify_slashable_attestation_signature(
    ...
    assert not verify_slashable_attestation_signature(
...
def _run_verify_slashable_vote(
    ...
    if should_succeed:
        validate_slashable_attestation(
            state, votes, max_indices_per_slashable_vote, slots_per_epoch
        )
    else:
        with pytest.raises(ValidationError):
            validate_slashable_attestation(
...
def test_verify_slashable_attestation_after_fork(
    ...
    _run_verify_slashable_vote(
        slots_per_epoch, valid_params, state, max_indices_per_slashable_vote, True
    )
