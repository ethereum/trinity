...
class DiscV5Plugin(Service):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.datagram_sender = DatagramSender(
            outgoing_datagram_channels[1],
            self.socket,
        )
        self.datagram_receiver = DatagramReceiver(
            self.socket,
    ...
    async def run(self) -> None:
        ...
        async with trio.open_nursery() as nursery:
            for service in self.services:
                nursery.start_soon(Manager.run_service, service)
...
async def run_discv5(config: Dict[str, Any]) -> None:
    ...
    await Manager.run_service(plugin)
