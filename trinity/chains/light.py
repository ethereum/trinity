...
from eth.chains.base import AccountState
from eth.db.backends.base import BaseDB
from eth.db.chain import BaseChainDB
from eth.db.header import BaseHeaderDB
from eth.rlp.blocks import BaseBlock
from eth.rlp.headers import BlockHeader, HeaderParams
...
class LightDispatchChain(BaseAsyncChain):
    ...
    def get_chaindb_class(cls) -> Type[BaseChainDB]:
        raise NotImplementedError(
            "Chain classes must implement " + inspect.stack()[0][3]
        )
    ...
    async def coro_get_block_by_header(self, header: BlockHeader) -> BaseBlock:
        ...
        return block_class(
            header=header, transactions=transactions, uncles=block_body.uncles
        )
    ...
    def build_block_with_transactions(
        ...
        BaseBlock, Tuple[Receipt, ...], Tuple[BaseComputation, ...]
    ]:  # noqa: E501
        raise NotImplementedError(
            "Chain classes must implement " + inspect.stack()[0][3]
        )
    ...
    def get_transaction_result(
        self, transaction: BaseOrSpoofTransaction, at_header: BlockHeader
    ) -> bytes:
        raise NotImplementedError(
            "Chain classes must implement " + inspect.stack()[0][3]
        )
    ...
    def estimate_gas(
        self, transaction: BaseOrSpoofTransaction, at_header: BlockHeader = None
    ) -> int:
        raise NotImplementedError(
            "Chain classes must implement " + inspect.stack()[0][3]
        )
