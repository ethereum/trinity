...
class PeerDBServer(BaseService):
    ...
    async def handle_track_peer_event(self) -> None:
        async for command in self.wait_iter(self.event_bus.stream(TrackPeerEvent)):
            self.tracker.track_peer_connection(command.remote, command.is_outbound)

    async def handle_track_peer_meta_event(self) -> None:
        async for command in self.wait_iter(self.event_bus.stream(TrackPeerMetaEvent)):
            self.tracker.update_peer_meta(
                command.remote,
    ...
    async def _run(self) -> None:
        ...
        self.run_daemon_task(self.handle_track_peer_meta_event())
