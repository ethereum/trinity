...
from trinity._utils.ipc import kill_process_id_gracefully, remove_dangling_ipc_files
...
class FixUncleanShutdownPlugin(BaseMainProcessPlugin):
    ...
    def fix_unclean_shutdown(
        self, args: Namespace, trinity_config: TrinityConfig
    ) -> None:
        self.logger.info("Cleaning up unclean shutdown...")

        self.logger.info(
            "Searching for process id files in %s..." % trinity_config.data_dir
        )
        pidfiles = tuple(trinity_config.pid_dir.glob("*.pid"))
        if len(pidfiles) > 1:
            self.logger.info(
                "Found %d processes from a previous run. Closing..." % len(pidfiles)
            )
        elif len(pidfiles) == 1:
