...
def _mk_minimum_viable_signed_beacon_blocks(
    ...
    for slot in slots:
        ...
        state, block = state_machine_provider(slot).apply_state_transition(state, block)
        yield block
...
async def test_beacon_node_can_track_canonical_head_by_highest_slot(
    ...
    number_of_slots = 10
    blocks = _build_branch_across_slots(
        number_of_slots, beacon_node._chain, eth2_config
    )
    for block in blocks:
