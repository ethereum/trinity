...
@pytest.mark.parametrize(
    ("validator_indices", "are_validator_indices_ascending"),
    [((0, 1, 2), True), ((0, 2, 1), False)],
)
def test_is_validator_indices_ascending(
    ...
    slashable_attestation = SlashableAttestation(
        **sample_slashable_attestation_params
    ).copy(validator_indices=validator_indices)
    assert (
        slashable_attestation.are_validator_indices_ascending
        == are_validator_indices_ascending
    )


@pytest.mark.parametrize(
    ("validator_indices", "custody_bitfield", "custody_bit_indices"),
    [((0, 1, 2), b"\x01", ((1, 2), (0,))), ((0, 1, 2), b"\x03", ((2,), (0, 1)))],
)
def test_custody_bit_indices(
    ...
    slashable_attestation = SlashableAttestation(
        **sample_slashable_attestation_params
    ).copy(validator_indices=validator_indices, custody_bitfield=custody_bitfield)
