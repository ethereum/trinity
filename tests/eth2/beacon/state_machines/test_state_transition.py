...
def test_per_slot_transition(
    base_db, genesis_block, genesis_state, fixture_sm_class, config, state_slot, keymap
):
    ...
    block = create_mock_block(
        state=state,
        config=config,
        state_machine=fixture_sm_class(chaindb, genesis_block),
    ...
    sm = fixture_sm_class(chaindb, block)
    ...
    # latest_block_roots
    latest_block_roots_index = (
        updated_state.slot - 1
    ) % st.config.SLOTS_PER_HISTORICAL_ROOT
    assert (
        updated_state.latest_block_roots[latest_block_roots_index]
