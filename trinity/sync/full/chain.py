...
from eth_utils import humanize_hash, humanize_seconds, ValidationError
from eth_utils.toolz import concat, first, groupby, merge, valfilter

from eth.constants import BLANK_ROOT_HASH, EMPTY_UNCLE_HASH
...
from trinity.protocol.eth.constants import MAX_BODIES_FETCH, MAX_RECEIPTS_FETCH
from trinity.protocol.eth.peer import ETHPeer, ETHPeerPool
from trinity.protocol.eth.sync import ETHHeaderChainSyncer
from trinity.rlp.block_body import BlockBody
from trinity.sync.common.chain import BaseBlockImporter, SimpleBlockImporter
from trinity.sync.common.constants import EMPTY_PEER_RESPONSE_PENALTY
...
from trinity._utils.headers import skip_complete_headers
...
BlockBodyBundle = Tuple[BlockBody, Tuple[Hash32, Dict[Hash32, bytes]], Hash32]
...
class BaseBodyChainSyncer(BaseService, PeerSubscriber):
        ...
        async for headers in self.wait_iter(get_headers_coro):
            ...
            try:
                ...
                try:
                    ...
                    await self._log_missing_parent(
                        new_headers[0], highest_block_num, missing_exc
                    )
    ...
    async def _block_body_bundle_processing(
        self, bundles: Tuple[BlockBodyBundle, ...]
    ) -> None:
        """
    ...
    async def _run_body_download_batch(
        self, peer: ETHPeer, batch_id: int, all_headers: Tuple[BlockHeader, ...]
    ) -> None:
        """
        ...
        non_trivial_headers = tuple(
            header for header in all_headers if not _is_body_empty(header)
        )
        trivial_headers = tuple(
            header for header in all_headers if _is_body_empty(header)
        )
        ...
        try:
            ...
            self.logger.info(
                "Unexpected p2p perror while downloading body from peer: %s", exc
            )
            self.logger.debug(
                "Problem downloading body from peer, dropping...", exc_info=True
            )
        else:
            if len(non_trivial_headers) == 0:
                ...
                self.logger.debug(
                    "Pausing %s for %.1fs, for sending 0 block bodies", peer, delay
                )
    ...
    async def _get_block_bodies(
        self, peer: ETHPeer, headers: Tuple[BlockHeader, ...]
        ...
        bodies_by_root = {
            ...
            for block_body, (transaction_root, _), uncles_hash in block_body_bundles
        }

        header_roots = {
            header: (header.transaction_root, header.uncles_hash) for header in headers
        }

        completed_header_roots = valfilter(
            lambda root: root in bodies_by_root, header_roots
        )
    ...
    async def _request_block_bodies(
        self, peer: ETHPeer, batch: Tuple[BlockHeader, ...]
    ) -> Tuple[BlockBodyBundle, ...]:
        """
        ...
        """
        self.logger.debug(
            "Requesting block bodies for %d headers from %s", len(batch), peer
        )
        try:
    ...
    async def _log_missing_parent(
        self, first_header: BlockHeader, highest_block_num: int, missing_exc: Exception
    ) -> None:
        self.logger.warning(
            "Parent missing for header %r, restarting header sync", first_header
        )
        ...
        try:
            ...
            self.logger.debug(
                "Could not find canonical header at #%d: %s", block_num, exc
            )
            local_header = None
        ...
        try:
            ...
            self.logger.debug(
                "Could not find canonical header parent at #%d: %s", block_num, exc
            )
            local_parent = None
...
class FastChainSyncer(BaseService):
    def __init__(
        ...
        self._body_syncer = FastChainBodySyncer(
            chain, db, peer_pool, self._header_syncer, self.cancel_token
        )
...
class FastChainBodySyncer(BaseBodyChainSyncer):
    ...
    async def _launch_prerequisite_tasks(self) -> None:
        ...
        async for headers in self._sync_from_headers(
            self._block_persist_tracker, self._should_skip_header
        ):

            # Sometimes duplicates are added to the queue, when switching from one sync to another.
            # We can simply ignore them.
            new_body_tasks = tuple(
                h for h in headers if h not in self._block_body_tasks
            )
            new_receipt_tasks = tuple(
                h for h in headers if h not in self._receipt_tasks
            )
        ...
        while self.is_operational:
            ...
            self.logger.debug(
                ...
                [
                    ...
                    for q in (self._block_body_tasks, self._receipt_tasks)
                ],
    ...
    async def _persist_ready_blocks(self) -> None:
        ...
        while self.is_operational:
            ...
            get_completed_coro = self._block_persist_tracker.ready_tasks(
                BLOCK_QUEUE_SIZE_TARGET
            )
    ...
    async def _persist_blocks(self, headers: Tuple[BlockHeader, ...]) -> None:
        ...
        for header in headers:
            ...
            if _is_body_empty(header):
                ...
                transactions = [
                    tx_class.from_base_transaction(tx) for tx in body.transactions
                ]
    ...
    def _mark_body_download_complete(
        ...
        self._block_persist_tracker.finish_prereq(
            BlockPersistPrereqs.StoreBlockBodies, completed_headers
        )
    ...
    async def _run_receipt_download_batch(
        ...
        try:
            ...
            self._block_persist_tracker.finish_prereq(
                BlockPersistPrereqs.StoreReceipts, completed_headers
            )
        except BaseP2PError as exc:
            self.logger.info(
                "Unexpected p2p perror while downloading receipt from peer: %s", exc
            )
            self.logger.debug(
                "Problem downloading receipt from peer, dropping...", exc_info=True
            )
        else:
            ...
            if len(completed_headers) > 0:
                ...
                delay = EMPTY_PEER_RESPONSE_PENALTY
                self.logger.debug(
                    "Pausing %s for %.1fs, for sending 0 receipts", peer, delay
                )
    ...
    async def _process_receipts(
        self, peer: ETHPeer, all_headers: Tuple[BlockHeader, ...]
    ) -> Tuple[BlockHeader, ...]:
        """
        ...
        unique_headers_needed = tuple(
            first(headers) for root, headers in receipt_root_to_headers.items()
        )
        ...
        try:
            ...
            self.logger.info(
                "Disconnecting from %s: sent invalid receipt: %s", peer, err
            )
    ...
    async def _validate_receipts(
        ...
        receipt_bundles: Tuple[ReceiptBundle, ...],
    ) -> None:
    ...
    async def _request_receipts(
        ...
        """
        self.logger.debug(
            "Requesting receipts for %d headers from %s", len(batch), peer
        )
        try:
...
class RegularChainBodySyncer(BaseBodyChainSyncer):
    ...
    def _mark_body_download_complete(
        ...
        self._block_import_tracker.finish_prereq(
            BlockImportPrereqs.StoreBlockBodies, completed_headers
        )
    ...
    async def _import_blocks(self, headers: Tuple[BlockHeader, ...]) -> None:
        ...
        for header in headers:
            ...
            if new_canonical_blocks == (block,):
                ...
                self.logger.info(
                    "Imported block %d (%d txs) in %.2f seconds",
                    block.number,
                    len(transactions),
                    timer.elapsed,
                )
...
def _is_body_empty(header: BlockHeader) -> bool:
    return (
        header.transaction_root == BLANK_ROOT_HASH
