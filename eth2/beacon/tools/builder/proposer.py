...
def _generate_randao_reveal(
    privkey: int, slot: Slot, fork: Fork, config: Eth2Config
) -> BLSSignature:
    """
...
def validate_proposer_index(
    ...
    beacon_proposer_index = get_beacon_proposer_index(
        state.copy(slot=slot), slot, CommitteeConfig(config)
    )
...
def _get_proposer_index(
    state: BeaconState, slot: Slot, config: Eth2Config
) -> ValidatorIndex:
    proposer_index = get_beacon_proposer_index(state, slot, CommitteeConfig(config))
...
def create_mock_block(
    *,
    ...
    proposer_index = _get_proposer_index(future_state, slot, config)
