...
from trinity.config import TrinityConfig, BeaconAppConfig
from trinity.constants import IPC_DIR, LOG_DIR, LOG_FILE, PID_DIR
from trinity._utils.filesystem import is_under_path


@pytest.mark.parametrize(
    "app_identifier, expected_suffix", (("beacon", "-beacon"), ("", ""))
)
def test_trinity_config_app_identifier(
    xdg_trinity_root, app_identifier, expected_suffix
):

    data_dir = get_local_data_dir("muffin", xdg_trinity_root)
    trinity_config = TrinityConfig(
        network_id=1, data_dir=data_dir, app_identifier=app_identifier
    )

    assert trinity_config.network_id == 1
    assert trinity_config.data_dir == data_dir
    assert (
        trinity_config.logfile_path == data_dir / (LOG_DIR + expected_suffix) / LOG_FILE
    )
    assert (
        trinity_config.jsonrpc_ipc_path
        == data_dir / (IPC_DIR + expected_suffix) / JSONRPC_SOCKET_FILENAME
    )  # noqa: E501
    assert (
        trinity_config.database_ipc_path
        == data_dir / (IPC_DIR + expected_suffix) / DATABASE_SOCKET_FILENAME
    )  # noqa: E501
...
def test_trinity_config_explicit_properties():
    trinity_config = TrinityConfig(
        network_id=1, data_dir="./data-dir", nodekey_path="./nodekey"
    )
...
def test_trinity_config_nodekey_loading(nodekey_bytes, nodekey_path):
    trinity_config = TrinityConfig(network_id=1, nodekey_path=nodekey_path)
