...
from eth2._utils.bitfield import get_bitfield_length, get_empty_bitfield, set_voted
...
@pytest.mark.parametrize(("genesis_epoch"), [(0)])
@pytest.mark.parametrize(
    ("epoch", "previous_epoch", "next_epoch", "success"),
    [
        (0, 0, 1, True),
        (0, 0, 2, True),
...
def test_validate_epoch_within_previous_and_next(
    epoch, previous_epoch, next_epoch, success, slots_per_epoch, genesis_epoch
):
    if success:
