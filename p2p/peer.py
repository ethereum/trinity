...
from p2p.abc import CommandAPI, ConnectionAPI, NodeAPI, ProtocolAPI
...
class BasePeer(BaseService):
    ...
    def __init__(self,
        ...
        self.base_protocol = self.connection.base_protocol
        ...
        self.client_version_string = self.connection.client_version_string
    ...
    async def _run(self) -> None:
        ...
        # setup handler for all non-p2p propocol messages to pass messages to subscribers
        for protocol in self.multiplexer.get_protocols():
            if protocol.name == 'p2p':
                continue
            self.connection.add_protocol_handler(protocol.name, self._handle_subscriber_message)
    ...
    async def _handle_subscriber_message(self, cmd: Command, msg: Payload) -> None:
