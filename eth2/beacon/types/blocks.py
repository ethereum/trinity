...
class BeaconBlockBody(ssz.Serializable):
    ...
    def is_empty(self) -> bool:
        return (
            self.randao_reveal == EMPTY_SIGNATURE
            and self.eth1_data == Eth1Data.create_empty_data()
            and self.proposer_slashings == ()
            and self.attester_slashings == ()
            and self.attestations == ()
            and self.deposits == ()
            and self.voluntary_exits == ()
            and self.transfers == ()
        )

    @classmethod
    def cast_block_body(cls, body: "BeaconBlockBody") -> "BeaconBlockBody":
        return cls(
