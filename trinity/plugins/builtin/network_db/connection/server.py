...
class ConnectionTrackerServer(BaseService):
    ...
    async def handle_should_connect_to_requests(self) -> None:
        async for req in self.wait_iter(
            self.event_bus.stream(ShouldConnectToPeerRequest)
        ):
            ...
            self.event_bus.broadcast(
                ShouldConnectToPeerResponse(should_connect), req.broadcast_config()
            )
    ...
    async def handle_blacklist_command(self) -> None:
        async for command in self.wait_iter(self.event_bus.stream(BlacklistEvent)):
            ...
            self.tracker.record_blacklist(
                command.remote, command.timeout_seconds, command.reason
            )
