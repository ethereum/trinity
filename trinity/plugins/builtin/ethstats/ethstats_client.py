...
class EthstatsClient(BaseService):
    ...
    async def _run(self) -> None:
        await self.wait_first(self.send_handler(), self.recv_handler())
    ...
    def serialize_message(self, message: EthstatsMessage) -> str:
        return json.dumps(
            {"emit": [message.command, {**message.data, "id": self.node_id}]}
        )
    ...
    async def send_hello(self, secret: str, info: EthstatsData) -> None:
        await self.send_queue.put(
            EthstatsMessage("hello", {"info": info, "secret": secret})
        )

    async def send_stats(self, stats: EthstatsData) -> None:
        await self.send_queue.put(EthstatsMessage("stats", {"stats": stats}))

    async def send_block(self, block: EthstatsData) -> None:
        await self.send_queue.put(EthstatsMessage("block", {"block": block}))

    async def send_pending(self, pending: int) -> None:
        await self.send_queue.put(
            EthstatsMessage("pending", {"stats": {"pending": pending}})
        )

    async def send_history(self, history: EthstatsData) -> None:
        await self.send_queue.put(EthstatsMessage("history", {"history": history}))

    async def send_node_ping(self) -> None:
        await self.send_queue.put(
            EthstatsMessage("node-ping", {"clientTime": timestamp_ms()})
        )

    async def send_latency(self, latency: int) -> None:
        await self.send_queue.put(EthstatsMessage("latency", {"latency": latency}))
