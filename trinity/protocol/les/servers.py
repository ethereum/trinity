...
class LESPeerRequestHandler(BasePeerRequestHandler):
    async def handle_get_block_headers(
        self, peer: LESPeer, msg: Dict[str, Any]
    ) -> None:
        ...
        peer.sub_proto.send_block_headers(
            headers, buffer_value=0, request_id=request.request_id
        )
...
class LightRequestServer(BaseRequestServer):
    ...
    def __init__(
        self, db: BaseAsyncHeaderDB, peer_pool: LESPeerPool, token: CancelToken = None
    ) -> None:
        super().__init__(peer_pool, token)
        self._handler = LESPeerRequestHandler(db, self.cancel_token)

    async def _handle_msg(
        self, base_peer: BasePeer, cmd: Command, msg: _DecodedMsgType
    ) -> None:
