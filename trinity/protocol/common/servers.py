...
class BaseRequestServer(BaseService, PeerSubscriber):
    ...
    def __init__(self, peer_pool: BasePeerPool, token: CancelToken = None) -> None:
    ...
    async def _quiet_handle_msg(
        self, peer: BasePeer, cmd: protocol.Command, msg: protocol._DecodedMsgType
    ) -> None:
        try:
    ...
    async def _handle_msg(
        self, peer: BasePeer, cmd: Command, msg: _DecodedMsgType
    ) -> None:
        """
...
class BasePeerRequestHandler(CancellableMixin, HasExtendedDebugLogger):
    ...
    async def lookup_headers(
        self, request: BaseHeaderRequest
    ) -> Tuple[BlockHeader, ...]:
        """
        ...
        headers: Tuple[BlockHeader, ...] = tuple(
            [header async for header in self._generate_available_headers(block_numbers)]
        )
        return headers
    ...
    async def _generate_available_headers(
        self, block_numbers: Tuple[BlockNumber, ...]
