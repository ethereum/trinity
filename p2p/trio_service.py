...
from typing import Any, Callable, Awaitable, Optional, Tuple, Type, AsyncIterable
...
class ManagerAPI(ABC):
    ...
    @abstractmethod
    async def run_daemon_task(self, coro: Awaitable[Any], name: str = None) -> None:
        ...
...
class Service(ABC):
    ...
    def from_function(cls, logic_fn: LogicFnType) -> 'Service':
        """
        Create a service out of a simple
        """
        return type(
            logic_fn.__name__,
            (cls, FunctionalServiceMixin),
            {'run': staticmethod(logic_fn)},
        )


class FunctionalServiceMixin:
    logic_fn: LogicFnType
    ...
    async def run(self, manager: ManagerAPI) -> None:
        await self.logic_fn(manager, *self._args, **self._kwargs)
...
@asynccontextmanager
async def run_service(service: Service) -> AsyncIterable[ManagerAPI]:
    async with trio.open_nursery() as nursery:
