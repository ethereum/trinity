...
@pytest.mark.parametrize(("initiated_exit", "success"), [(False, True), (True, False)])
def test_validate_voluntary_exit_initiated_exit(genesis_state, initiated_exit, success):
    state = genesis_state
...
def test_validate_voluntary_exit_epoch(
    ...
    if success:
        validate_voluntary_exit_epoch(
            voluntary_exit, state.current_epoch(slots_per_epoch)
        )
    else:
        with pytest.raises(ValidationError):
            validate_voluntary_exit_epoch(
